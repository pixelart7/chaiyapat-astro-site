<script lang="ts">
  ...
  async function onClickCopy() {
    navigator.clipboard.writeText(window.location.href)
      .then(() => {
        isShowCopied = true
        setTimeout(() => { isShowCopied = false }, 3000)
      })
  }
...
{#if isShow}
  <li>
    ...
    <div class="flex text-sm font-medium gap-1">
      <button class="cursor-pointer h-6 flex items-center justify-center p-1 bg-gray-100 hover:bg-primary-500/10 rounded overflow-hidden" on:click={() => onClickCopy()}>
        <span class="w-4 relative">
          <svg class="absolute inset-0 bg-gray-100 transition-all { isShowCopied ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1 scale-[0.98]' }" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 256 256" {...$$props}><path fill="currentColor" d="m232.49 80.49l-128 128a12 12 0 0 1-17 0l-56-56a12 12 0 1 1 17-17L96 183L215.51 63.51a12 12 0 0 1 17 17Z"/></svg>
        ...
        <span class="ml-1 relative">
          <span class="absolute inset-0 bg-gray-100 transition-all { isShowCopied ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1 scale-[0.98]' }">Copied!</span>
          Copy Link
        </span>
      </button>
      <button class="cursor-pointer h-6 flex items-center justify-center p-1 bg-gray-100 hover:bg-primary-500/10 rounded" on:click={() => onClickShare()}>
