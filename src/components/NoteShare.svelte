...
{#if isShow}
  <li>
    ...
    <div class="flex text-sm font-medium gap-1">
      <button class="cursor-pointer h-6 flex items-center justify-center p-1 bg-gray-100 dark:bg-gray-800 dark:hover:bg-primary-500/10 hover:bg-primary-500/10 rounded overflow-hidden" on:click={() => onClickCopy()}>
        ...
        <span class="ml-1 relative">
          <span class="absolute inset-0 bg-gray-100 dark:bg-gray-900 transition-all { isShowCopied ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1 scale-[0.98]' }">Copied!</span>
          Copy Link
        </span>
      </button>
